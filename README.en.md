...
Chinese: [README.md](README.md) · Legacy Chinese link: [README.zh-CN.md](README.zh-CN.md) · [Cases](docs/cases/README.md) · [Contributing](CONTRIBUTING.md) · [Cross-repository navigation](docs/cross-repo-navigation.md)

![Three minimal iteration sketches show an initial version, a failed middle version, and a corrected final version / 三张极简迭代草图展示初版、中间失败版和最终修正版。](docs/assets/social-preview.png)

This repository package documents a cover-system experiment: 75+ deterministic HTML layout passes, 147 ordinary/debug process images, edge evidence images, source code, and a reusable agent skill for making Xiaohongshu-style cover titles without copying third-party screenshots or licensed characters. Those counts are process evidence, not records of publication, reach, engagement, growth, recommendation, or commercial outcome.
...
## Cases, status, and boundary

Read the [v59 → v74 → v75 case](docs/cases/v59-v74-v75.md) and the [object-carrier case](docs/cases/object-carrier.md) for problem → input → method → reproducible output → evidence → unsupported conclusions → three-minute experience.

This is a repository package with source code, bilingual documentation entry points, process assets, and a reusable skill. The visual system remains a living experiment, not a final template. Nothing here claims that a cover was posted, passed platform review, or created a performance result.

Public maintainer identity: 马智辰 (Zhichen Ma / 智辰老师) · soarhigh1991@gmail.com · X [@AI2studio](https://x.com/AI2studio)
